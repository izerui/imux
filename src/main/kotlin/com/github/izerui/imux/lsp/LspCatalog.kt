...
internal object LspCatalog {
    /**
     * 已核实的安装命令。命令是否可跨平台由首个 token 决定：以 npm、go、dotnet、
     * rustup 或 gem 开头的 server 安装命令可跨平台执行；以 brew 或 opam 开头的命令
     * 只在 macOS 放行。安装这些工具本身可能仍依赖 brew，不能与 server 命令混为一谈。
     */
    val servers: Map<String, LspServer> =
        listOf(
            // 以下六条已通过本机安装或上游包管理器元数据核实
     ...
     *
     * 判据是命令的第一个 token 是 `brew` 还是 `opam`。语言服务器安装命令中，以
     * npm / go / gem / dotnet / rustup 开头的 8 条可跨平台执行；brew / opam 命令则
     * 只在 macOS 放行。工具自身的安装命令也使用同一判据，因此例如
     * `brew install rustup` 不会因为最终提供的是跨平台工具而被误放行。
     *
     ...
     * 非 macOS 上仍保留短目标名、完整命令 tooltip 与上游文档链接，明确说明自动执行
     * 边界之外的下一步。
     *
     ...
     */
    val languages: List<LspLanguage> =
        listOf(
