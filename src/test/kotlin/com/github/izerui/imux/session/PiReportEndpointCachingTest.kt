...
class PiReportEndpointCachingTest {
    ...
    fun `阻塞的启动等待只出现在后台协程里`() {
        ...
        assertTrue(
            ...
            Regex("""launch\(Dispatchers\.IO\)\s*\{\s*cached = compute\(\)""")
    ...
    fun `读取端点绝不等待`() {
        ...
        assertFalse(
            "不能在读取路径上 await / join / runBlocking",
            endpoint.contains("runBlocking") ||
                endpoint.contains(".await()") ||
    ...
    fun `启动时预热缓存`() {
        ...
        assertTrue(
            ...
            startup.contains("PiReportEndpointCache.warmUp()"),
        )
    }
}
