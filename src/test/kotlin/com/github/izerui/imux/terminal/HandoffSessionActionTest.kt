...
class HandoffSessionActionTest {
    ...
    fun `英文交接提示只有一句引导语和复制内容`() {
        assertEquals(
            "Task handoff: take over the task from the session below, read its full context, and assess the current progress; " +
    ...
    fun `中文交接提示只有一句引导语和复制内容`() {
        assertEquals(
            "任务转移：接手以下会话中的任务，读取完整上下文并梳理当前进展；" +
                "反馈结果并等待下一步指示。\n\n" +
    ...
    fun `交接菜单只生成已启用的 Agent`() {
        ...
        assertEquals(
            enabled.map { ImuxBundle.message("action.handoff.target.text", it.displayName) },
