...
曾实现过「● 标签页开着 / ○ 后台运行中 / 无标记」的三态标记，实际使用中未能
正确反映真实状态，已整体移除。

失败原因未查清。已知的判断依据有两处不可靠：一是仅凭「插件是否创建过终端」
来判断，CLI 自行退出后仍会谎报运行中；二是改用 `TerminalView.sessionState`
后行为依旧不对，怀疑与 `closeOnProcessTermination(false)` 有关——该选项让
进程结束后 UI 保留，状态可能因此不转为 `Terminated`。**此结论未经验证。**

后续若要在此处做标记，语义应改为「本轮对话交互是否已完成」（即 agent 是否
跑完、可以回来看了），而非「进程是否存活」。这需要先把「如何可靠获知 CLI 当前
状态」这个问题解决——上面那个未查清的疑点届时仍然拦路。
