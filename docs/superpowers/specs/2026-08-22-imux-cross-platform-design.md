...
| **codex** | `lsof -p` 取句柄 **不动** | `/proc/<pid>/fd/` 读软链 | **`-c hooks.SessionStart` → HTTP 上报** |
...
#### Windows / codex：`-c hooks.SessionStart` 注入

codex 既没有运行态文件（`state_5.sqlite` 的 `threads` 表无 pid 字段，本次复核仍成立），
...
`session/PiSessionReportHandler.kt` 现在只认一个路径、只调
`SessionMonitor.onPiSessionReported`。改动：
...
2. `session/LiveSessionProbe.kt:143` `fileNameOf` 同样只切 `/`。Windows 上 rollout 路径
   来自 codex 的 hook payload，用 `\`
...
| codex hook 注入被拒 / 未触发 | 同上，漂移探测不认领，会话正常 |
...
- **不把 hook 机制推广到 macOS/Linux 的 codex**。那会改动正在工作的路径，与统领约束
  冲突；它作为将来统一三平台的方案记入遗留
- **不给 claude 装 hook**。Windows 上 claude 走 pid 自报，够用且不依赖 claude 的特性

## 遗留

1. **codex 的 hook 上报应当取代三平台的 `ps`/`lsof`/`/proc` 轮询**。它是推而不是拉，
   更准更省。本次不做，因为会动到 macOS 上正在工作的路径
